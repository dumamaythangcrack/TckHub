--[[
    ========================================================================
    ⚡ TCK HUB PREMIUM UI LIBRARY - REDESIGNED VERSION 2026 ⚡
    ========================================================================
    * High-Performance Rendering & Modern Dark-Glow Aesthetics
    * Smooth Tween Animations & Responsive Physics Bypass Elements
    * Fully Compatible with previous TckHub API standard.
]]

-- Dọn dẹp các UI cũ nếu có tránh bị trùng lặp hiển thị
if (game:GetService("CoreGui")):FindFirstChild("TckHub") then
	(game:GetService("CoreGui")).TckHub:Destroy()
end
if (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
	-- Tránh destroy nhầm ScreenGui hệ thống, chỉ dọn ScreenGui tạo bởi loader cũ nếu khớp tên
	local oldScreen = (game:GetService("CoreGui")):FindFirstChild("ScreenGui")
	if oldScreen:FindFirstChild("OutlineButton") then
		oldScreen:Destroy()
	end
end

-- ==================== ĐỊNH NGHĨA TÔNG MÀU PREMIUM ====================
_G.Primary = Color3.fromRGB(160, 160, 165)   -- Màu phụ xám nhạt hiện đại
_G.Dark = Color3.fromRGB(15, 15, 18)         -- Nền siêu tối sang trọng
_G.Third = Color3.fromRGB(255, 30, 60)       -- Neon Đỏ rực rỡ làm điểm nhấn chính
local AccentGlow = Color3.fromRGB(255, 60, 90)
local SecondaryDark = Color3.fromRGB(25, 25, 30)
local TextWhite = Color3.fromRGB(245, 245, 250)
local TextMuted = Color3.fromRGB(140, 140, 145)

-- Hàm phụ trợ bo góc nhanh
...
-- Hàm tạo đường viền mỏng cao cấp (UIStroke)
...
-- HÀM KÉO THẢ UI MƯỢT MÀ (Tương thích cả PC và Touch di động)
function MakeDraggable(topbarobject, object)
	...
	local function Update(input)
		...
		local Tween = TweenService:Create(object, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Position = pos
		})
		Tween:Play()
	end
...
-- ==================== KHỞI TẠO NÚT MỞ NHANH (MINI BUTTON) ====================
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "TckHub_QuickOpen"
...
OutlineButton.Size = UDim2.new(0, 52, 0, 52)
CreateRounded(OutlineButton, 14)
local btnStroke = CreateStroke(OutlineButton, _G.Third, 1.5, 0.3)
...
ImageButton.ImageColor3 = TextWhite
ImageButton.Image = "rbxassetid://13940080072"
ImageButton.AutoButtonColor = false
CreateRounded(ImageButton, 10)
MakeDraggable(ImageButton, OutlineButton)

-- Hiệu ứng Hover nút mở nhanh
ImageButton.MouseEnter:Connect(function()
	TweenService:Create(OutlineButton, TweenInfo.new(0.2), {BackgroundColor3 = SecondaryDark}):Play()
	TweenService:Create(btnStroke, TweenInfo.new(0.2), {Color = AccentGlow, Thickness = 2}):Play()
end)
ImageButton.MouseLeave:Connect(function()
	TweenService:Create(OutlineButton, TweenInfo.new(0.2), {BackgroundColor3 = _G.Dark}):Play()
	TweenService:Create(btnStroke, TweenInfo.new(0.2), {Color = _G.Third, Thickness = 1.5}):Play()
end)

ImageButton.MouseButton1Click:Connect(function()
	local hub = game.CoreGui:FindFirstChild("TckHub")
	if hub then
		hub.Enabled = not hub.Enabled
		-- Hiệu ứng nảy nhẹ khi bấm
		OutlineButton:TweenSize(UDim2.new(0, 46, 0, 46), "Out", "Back", 0.1, true, function()
			OutlineButton:TweenSize(UDim2.new(0, 52, 0, 52), "Out", "Back", 0.15, true)
		end)
	end
end)

-- ==================== HỆ THỐNG THÔNG BÁO SIÊU ĐẸP ====================
...
local function RemoveOldestNotification()
	if #NotificationList > 0 then
		...
		TweenService:Create(removed[1], TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
...
function Update:Notify(desc)
	...
	OutlineFrame.Size = UDim2.new(0, 360, 0, 65)
	CreateRounded(OutlineFrame, 12)
	local notifStroke = CreateStroke(OutlineFrame, _G.Third, 1.5, 0.2)
...
(getgenv()).LoadConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("TckHub") then
			makefolder("TckHub")
		end
		if not isfolder("TckHub/Library/") then
			makefolder("TckHub/Library/")
		end
		local path = "TckHub/Library/" .. game.Players.LocalPlayer.Name .. ".json"
		...
		print("[TckHub] Config loaded successfully.")
	else
		return warn("[TckHub] Executor không hỗ trợ đọc/ghi file.")
	end
end;
...
(getgenv()).SaveConfig = function()
	if readfile and writefile and isfile and isfolder then
		...
		return warn("[TckHub] Executor không hỗ trợ lưu cấu hình.")
	end
end;
